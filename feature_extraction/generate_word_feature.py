            ...
            if index != 0:
                ...
                # else, take the nearest following ones
                    if len(word_feature[vid].keys()) != 0:
                        last_uttr = sorted(word_feature[vid].keys())[-1]
    ...
    with open(csv_file, 'r') as file:
        ...
        for row in filereader:
            ...
            if(index != 0):
                ...
                inputs = tokenizer(' '.join(uttr_tokens), return_tensors="tf")
                outputs = model(**inputs)
                word_feature[vid][uttr] = np.reshape(np.asarray(outputs.last_hidden_state), (np.asarray(outputs.last_hidden_state).shape[1], np.asarray(outputs.last_hidden_state).shape[2])).tolist()
...
#Extract word features using BERT
word_feature = {}
word_feature['Train'] = process_bert('./omg_TrainTranscripts.csv')
word_feature['Validation'] = process_bert('./omg_ValidationTranscripts.csv')

save_as_pkl(word_feature,'Word_Feature.pkl')

word_feature = {}
word_feature['Test'] = process_bert('./omg_TestTranscripts.csv')
save_as_pkl(word_feature,'Word_Feature_Test.pkl')   

#Exctract word featurest using opinion lexicon
# word_feature = {}
