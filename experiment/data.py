...
class DataSet():
    ...
    def __init__(self, istrain=True, model='trimodal', task='emotion', seq_length=20, model_name='trimodal', is_fusion=False, also_test=True):  # initialization, the length of sequences in one video is 20 (modifiable)
        ...
        if(model == 'word_feature'):
            self.train_video_csv_path = os.path.join('..',data_folder,'omg_TrainVideos.csv')
            self.validation_video_csv_path = os.path.join('..',data_folder,'omg_ValidationVideos.csv')

            self.test_video_csv_path = os.path.join('..',data_folder,'omg_TestVideos.csv')
        ...
        # where the audio_feature is stored
        self.audio_rnn_path = [os.path.join('..', data_folder, 'Audio_Feature.pkl'),
                                   os.path.join('..', data_folder, 'Audio_Feature_Test.pkl')] #OK
    ...
    def unroll_and_normalize_transformers(self, feature_dict):
        # So the normalization will be done in train set, validation set and (test set)
        normalized_f_dict = {}
        scaler = MinMaxScaler()
        if self.istrain:
        ...
        for state in states:
            ...
            for index in range(len(features)):
                if is_time:
                    # unrolled_f = np.asarray(features).reshape((-1,feature_dim))
                    if state == 'Train':
                        scaled_f = scaler.fit_transform(features[index])
                    else:
                        scaled_f = scaler.transform(features[index])
                else:
                    unrolled_f  = np.asarray(features)
                    if state == 'Train':
                        scaled_f = scaler.fit_transform(features[index])
                ...
                vid,uttr = indexes[index]
                main_dict[vid][uttr] = scaled_f
            normalized_f_dict[state] = main_dict
        return normalized_f_dict
    ...
    def load_word_feature(self):
        # This should be used for features extracted with lexicons
        # self.word_feature = self.unroll_and_normalize(self.load_feature(self.word_feature_path))

        #This should be used for features extracted with encoders from transformer architectures
        self.word_feature = self.unroll_and_normalize_transformers(self.load_feature(self.word_feature_path))
    ...
    def load_audio_feature(self):
        ...
        self.audio_feature = self.load_feature(self.audio_feature_path)
    ...
    def get_feature_size(self, feature):
        size_dict = {'bimodal':2, 'trimodal':3, '[':-1}
        size = 1
