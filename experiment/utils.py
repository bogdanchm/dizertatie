...
def pad_tensor(tensor, max_value):
    # pdb.set_trace()
    l = []
    for i in range(len(tensor)):
        if tensor[i].shape[0] <= max_value:
            l.append(np.concatenate((tensor[i], tensorflow.fill((max_value - tensor[i].shape[0], 768), 9000)), axis=0))
